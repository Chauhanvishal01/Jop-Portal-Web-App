...
export const fetchjobs =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `${import.meta.env.VITE_API_KEY}/api/v1/job/getall?`;
      let queryParams = [];
      if (searchKeyword) {
...
export const fetchSingleJob = (jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_KEY}/api/v1/job/get/${jobId}`,
...
export const postJob = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_KEY}/api/v1/job/post`,
      data,
      {
        withCredentials: true,
        headers: {
...
export const getMyJobs = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_KEY}/api/v1/job/getmyjobs`,
      { withCredentials: true }
    );
...
export const deleteJob = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_KEY}/api/v1/job/delete/${id}`,
      { withCredentials: true }
    );
