...
export const fetchEmployerApplications = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_KEY}/api/v1/application/employer/getall`,
      {
...
export const fetchJobSeekerApplications = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_KEY}/api/v1/application/jobseeker/getall`,
      {
        withCredentials: true,
      }
    );
...
export const postApplication = (data, jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_KEY}/api/v1/application/post/${jobId}`,
      data,
      {
        withCredentials: true,
...
export const deleteApplication = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_KEY}/api/v1/application/delete/${id}`,
      { withCredentials: true }
    );
    dispatch(
